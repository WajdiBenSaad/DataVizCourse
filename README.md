# DataVizCourse
My course on the Art of Data Visualisation

[![License: CC BY-NC 4.0](https://img.shields.io/badge/License-CC%20BY--NC%204.0-lightgrey.svg)](https://creativecommons.org/licenses/by-nc/4.0/)
![GitHub last commit](https://img.shields.io/github/last-commit/WajdiBenSaad/DataVizCourse)
![GitHub repo size](https://img.shields.io/github/repo-size/WajdiBenSaad/DataVizCourse) 
![Made with Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white) 
![Made with Javascript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black) 

## 📋 Table of Contents
- [Course Overview](#course-overview)
- [Course Structure](#course-structure)
- [Getting Started](#getting-started)
- [Citation](#citation)
- [Contributing](#contributing)
- [License](#license)
- [Contact](#contact)

## Citation
If you use this course in your research, teaching, or projects, please cite it as:

```latex
@misc{WajdiBenSaad_DataVizCourse,
    author       = {Wajdi BEN SAAD},
    title        = {Data Visualization Course},
    year         = {2025},
    publisher    = {GitHub},
    journal      = {GitHub repository},
    howpublished = {\url{hhttps://github.com/WajdiBenSaad/DataVizCourse#}},
    }
}
```
## Contributing

Contributions and feedback are welcome! Below are simple ways to help and how to submit changes.

### Ways to contribute
- Report bugs or open an issue: https://github.com/WajdiBenSaad/DataVizCourse/issues
- Suggest new topics, improvements, or examples
- Fix typos, formatting, or documentation errors via pull request
- Add examples, notebooks, or resources (place them in the appropriate folder and update the README)
- Share the course with colleagues and on social platforms

### How to submit changes
1. Fork the repository and create a feature branch.
2. Make changes, add tests/examples if relevant, and keep commits focused and descriptive.
3. Open a pull request that references any related issue and describes the change.
4. Respond to review comments and update the PR as needed.

### Style and license
- Follow existing code and documentation style (formatters/linting where applicable).
- Contributions are accepted under the repository license (CC BY‑NC 4.0) — ensure any added content respects third‑party licenses.

Thank you for helping improve this course!
## Contact

Feel free to reach out for questions, suggestions, or collaborations!
You can reach me on one of the socials below: 
[![Blogger](https://img.shields.io/badge/Blogger-FF5722?style=for-the-badge&logo=blogger&logoColor=white)](https://wajdix.blogspot.com/)
[![LinkedIn](https://img.shields.io/badge/linkedin-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/wajdibensaad/)
[![X](https://img.shields.io/badge/X-%23000000.svg?style=for-the-badge&logo=X&logoColor=white)](https://x.com/wajdi_bs)
[![Bluesky](https://img.shields.io/badge/Bluesky-0285FF?style=for-the-badge&logo=Bluesky&logoColor=white)](https://bsky.app/profile/wajdi.bsky.social)
[![Reddit](https://img.shields.io/badge/Reddit-%23FF4500.svg?style=for-the-badge&logo=Reddit&logoColor=white)](https://www.reddit.com/user/wajdix/)
[![Google Scholar](https://img.shields.io/badge/Google%20Scholar-4285F4?style=for-the-badge&logo=google-scholar&logoColor=white)](https://scholar.google.com/citations?user=LUOsfhYAAAAJ&hl=en)